...
Next we have a series of imports. Where we import something 'qualified' we are giving
the module we imported a name that we can refer to it by rather than bringing
everything it exports into scope. Where we import the module name followed by parentheses, such as with replicateM or liftIO, we are saying we only want to import the functions or values of that name and nothing else. In the case of 'import Web.Scotty', we are importing everything 'Web.Scotty' exports. An unqualified and unspecific import should be avoided except in cases where it will be *very* obvious where a function came from or because it's a toolkit you must use all together like Scotty.
...
First we need to generate our shortened URLs that will refer to the links people post to the service. 'alphaNum' is a 'String' of the characters we want to select from - an alphabet if you will.
...
With 'saveURI' I need passed as arguments my connection to Redis, the key I am setting
in Redis, the value I am setting the key to, and I perform side effects in IO to get
'Either R.Reply R.Status'. The key in this case is the randomly generated URI I created
...
In the case of 'getURI' I am just passing it the connection to Redis and the shortened URI key so that we can get the URI associated with that short URL and show the users where they're headed.

> getURI  :: R.Connection -> BC.ByteString -> IO (Either R.Reply (Maybe BC.ByteString))
> getURI conn shortURI = R.runRedis conn $ R.get shortURI

'main' is a function like any other, here it returns 'IO ()' and acts as the entrypoint
for our webserver when we start the executable. First we begin by invoking 'scotty 3000', this is a helper function from Scotty which given a port to run on and a Scotty application, will listen for requests and respond to them.

> main :: IO ()
> main = scotty 3000 $ do

First, outside of any specific route handlers, I am cheating a bit and binding my
...
a feel for how this works and why it's desirable. Briefly - it saves me a bunch of
'lift . lift . lift' etc.
...
We're matching on the 'Just' constructor of Maybe, we're throwing away the contents
because we only used 'parseURI' to check if the URI was correct, we don't actually
care to use the parsed URI object.

>       Just _  -> do

Here we are calling in our old friend 'shortyGen' to get a randomized URI to assign
...
We just use a 'let' expression with 'let shorty = BC.pack shawty' because it's not
participating in the enclosing monad, it's just packing the 'String' into a 'ByteString'.

>         let shorty = BC.pack shawty

Here we're encoding the uri the user passed us as a 'ByteString' as that's what the
Redis client library wants. Before it can be encoded, we had to change it from a lazy
...
We're just stringifying 'resp' which is 'Either R.Reply R.Status'. It'll be 'Right Ok ...yadda yadda' if it succeeded, 'Left ...' if it failed. We concatenate the stringified
...
Back to handling the 'Nothing' case from earlier, this happens if parseURI was given
...
Another http get request handler, this time with a parameterized URL component which
we have the name 'short'. This is so a request against '/EFG3YLB' will parse out
'EFG3YLB' as the param 'short'.

>   get "/:short" $ do

Same as before, 'param' does a bit of magic for us. 'param' handles checking multiple
sources of arguments for us. Previously we were getting a parameter from the HTTP GET
arguments, this time it's a component of the request path.

>     short <- param "short"

Another lifting of an IO action into ActionT. This time we're taking the shortened URI
...
First we case match on 'uri' over the 'Left' and 'Right' constructors of the Either
sum type. If we got 'Left'...well we're done. Show the user what Redis had to say
about it and give up.
...
Our value was 'Right', meaning the Either value was a "success" idiomatically speaking.
Here 'mbBS' is the 'Maybe BC.ByteString' that was inside of
'Either R.Reply (Maybe BC.ByteString)'. If we got 'Nothing' that means Redis didn't have
